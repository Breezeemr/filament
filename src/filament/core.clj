...
(def ^:private manifold-deferred?-fn
  ;; Lazily resolve manifold.deferred/deferred? if (and only if) manifold is
  ;; on the classpath. filament.core itself never references manifold by
  ;; name, so loading this namespace on a manifold-less classpath is fine.
  ;; The resolution is cached in a delay so we pay the lookup cost once.
  (delay
    (try
...
(defn deferred?
  "True if `x` is something filament knows how to deref as a deferred.
   Recognises Filament directly, and — when manifold is on the classpath —
   any `manifold.deferred/IDeferred` via a lazily-resolved delegate."
  [x]
  (or (instance? Filament x)
      (when-let [f @manifold-deferred?-fn]
        (boolean (f x)))))
...
(defn realized?
  "Delegates to the underlying CompletableFuture."
  [^Filament d]
  (.isDone ^CompletableFuture (.cf d)))
...
(defmacro let-flow
  ...
  (let [pairs   (partition 2 bindings)
        derefed (mapcat (fn [[sym expr]]
                          [sym `(filament.core/deref-if-deferrable ~expr)])
                        pairs)]
...
(defmacro loop
  "Manifold-compat alias: wraps `clojure.core/loop` in a `filament` body
   so `(f/loop [i 0] ...)` reads like `manifold.deferred/loop`. On a
