...
(deftest in-error-propagates-with-vthread-frames
  (let [d (t/in 10 (fn [] (throw (ex-info "boom-in" {}))))
        e (try @d ::no-throw (catch Throwable t t))]
    (is (instance? clojure.lang.ExceptionInfo e))
    ...
    (let [frames (map str (.getStackTrace e))]
      (is (some #(re-find #"filament.deferred_test" %) frames)))))

(deftest in-cancel-before-tick-prevents-run
  (let [ran (atom false)
        d   (t/in 200 (fn [] (reset! ran true) :late))]
    (f/cancel! d)
    (Thread/sleep 300)
    (is (false? @ran))
    (is (thrown? java.util.concurrent.CancellationException @d))))

(deftest every-body-throws-deschedules
  ;; Pin the contract: a throw inside a tick body deschedules t/every and
  ;; surfaces the throwable as the returned Filament's error, matching
  ;; manifold.time/every.
  (let [hits (atom 0)
        d    (t/every 20 (fn []
                           (swap! hits inc)
                           (throw (ex-info "tick-boom" {}))))]
    (is (thrown-with-msg? clojure.lang.ExceptionInfo #"tick-boom" @d))
    (let [seen @hits]
      (Thread/sleep 80)
      (is (= seen @hits)
          (str "expected the schedule to stop after the first throw; hits=" @hits)))))
